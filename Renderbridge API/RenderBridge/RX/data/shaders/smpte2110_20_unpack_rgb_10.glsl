...
// integer image
layout(binding = 0, rgb10_a2ui) uniform restrict writeonly uimage2D u_destination;
...
uvec3 fetch_components(uint bit_pos) {
  ...
  return (value.xxx >> uvec3(20, 10, 0)) & uvec3(0x3ff);
}
...
void store_data(uint offset) {
  ...
  imageStore(u_destination, ivec2(outpos), uvec4(rgb, 3));
}
