...
struct WarpBlendViewProjectionReveal {
  sampler2D sampler;
...
vec4 texture(WarpBlendViewProjectionReveal s, vec2 tex_coords) {
  vec4 color = vec4(1);
  ...
  if (s.warp_enabled) {
    ...
    warp_coords = vec2(pos_projected.x / pos_projected.w / 2.0 + 0.5, pos_projected.y / pos_projected.w / 2.0 + 0.5);
