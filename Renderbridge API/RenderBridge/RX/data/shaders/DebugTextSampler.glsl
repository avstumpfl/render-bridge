...
const uvec4 ascii[] = uvec4[](
  ...
  uvec4(0x000000,0x000000,0x000000,0x000000),//@ missing
...
int get_byte(int i, int n) {
  return (i >> (n * 8)) & 0xFF;
}

vec4 texture(DebugTextSampler s, vec2 tex_coords) {
  const vec2 char_size = vec2(8, 12);  
  vec2 uv = floor(tex_coords * s.scale);
  vec2 pos = uv - s.offset;
  
  if (pos.x >= 0 &&
      pos.y < char_size.y) {
    for (int i = 0; i < s.text_length; ++i) {
      int c = get_byte(s.text[i / 4], i % 4);
      if (c == 10) {
        pos.y += char_size.y;
        pos.x = uv.x - s.offset.x;      
      }
      else if (all(greaterThanEqual(pos, vec2(0))) &&
               all(lessThan(pos, char_size))) {
        int bit = int(pos.y * char_size.x + char_size.x - pos.x - 1.0);
        uvec4 sprite = ascii[c - 32];        
        if (bitfieldExtract(sprite.x, bit - 72, 1) != 0 ||
            bitfieldExtract(sprite.y, bit - 48, 1) != 0 ||
            bitfieldExtract(sprite.z, bit - 24, 1) != 0 ||
            bitfieldExtract(sprite.w, bit - 00, 1) != 0)
          return s.color;
