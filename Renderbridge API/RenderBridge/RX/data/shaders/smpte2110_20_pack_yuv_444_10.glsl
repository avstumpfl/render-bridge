...
vec3 rgb_to_yuv(vec3 rgb) {
  return (u_color_model_matrix * vec4(rgb, 1.0)).xyz;
}

uint pack_444_to_uint(vec3 color) {
  uvec3 tmp = uvec3(color * vec3(1023, 1023, 1023));
  return (tmp.x << 10) | (tmp.y << 20) | (tmp.z << 0);  
}
...
void main() {
  ...
  for (uint i = 0; i < 16; ++i)
    unpacked_data[gl_LocalInvocationID.x + i * 32][gl_LocalInvocationID.y] =
      pack_444_to_uint(rgb_to_yuv(load_data(in_block_pos, uvec2(i * 32, 0))));
